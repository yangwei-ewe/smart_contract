...
// const ccp = JSON.parse(readFileSync('connect-org1.json', 'utf8'));

const channelName = envOrDefault('CHANNEL_NAME', 'myfirstchannel');
const chaincodeName = envOrDefault('CHAINCODE_NAME', 'carbon_cc1');
...
const keyDirectoryPath = envOrDefault('KEY_DIRECTORY_PATH', path.resolve(cryptoPath, 'users', 'Admin@org1.example.com', 'msp', 'keystore'));

// Path to user certificate directory.
const certDirectoryPath = envOrDefault('CERT_DIRECTORY_PATH', path.resolve(cryptoPath, 'users', 'Admin@org1.example.com', 'msp', 'signcerts'));
...
async function main(): Promise<void> {
    ...
    try {
        ...
        await contract.submit('snapshotLedger');
    } finally {
        gateway.close();
        client.close();
    }
